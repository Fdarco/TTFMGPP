...
def processRou(row: list, tm: TTFM, i: int, N: int, stime):
    pg = i/N
    dt = row[0]
    rou = row[1].split(' ')
    ...
    try:
        ...
        ctime = time.time()
        print('Progress: %.6f; Cost: %.4f s' % (pg, ctime-stime))
        return
    routeEdges = ' '.join(sroute)
    ctime = time.time()
    print('Progress: %.6f; Cost: %.4f s' % (pg, ctime-stime))
    return [dt, routeEdges]
...
def PPPRunner(tm: TTFM, routesInfo: list):
    ...
    pool = Pool(10)
    stime = time.time()
    rl = [[routesInfo[i], tm, i, len(routesInfo), stime] for i in range(len(routesInfo))]
    pr = pool.starmap(processRou, rl)

    plannedRoutes = pr

    with open('PPRoutes.pickle', 'wb') as f:
        pickle.dump(plannedRoutes, f)

    plannedRoutes.sort(key=lambda x: x[0])
...
if __name__ == '__main__':
    ...
    historicalTTFM = TTFM('lima', 25000, 5)
    TTFMRes(historicalTTFM, routesInfo)
    print('Historical TTFM generated successfully.')


    todayRoutes = getRouInfo('../SUMOFiles/lima.rou.xml')
    PPRoutes = PPPRunner(historicalTTFM, todayRoutes)
